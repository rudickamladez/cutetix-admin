      ...
      next: event_from_api => {
        ...
        this.#eventService.capacitySummaryById(this.event_id!).subscribe({
          ...
          next: eventCapacitySum => {
            ...
            this.#logging.log("dashboardEventLoad", `Loaded successfully capacity summary for ${this.event?.name}`);
          },
          // error
          error: err => {
            this.#logging.error("dashboardEventLoad", `Cannot load event capacity summary for ${this.event?.name}`);
            this.#toastr.error(err.message, "Cannot load event capacity summary", {
              progressBar: true,
            });
            return;
          },

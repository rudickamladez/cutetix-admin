...
export class TicketService {
  ...
  public create(ticket: Ticket): Observable<Ticket> {
    return this.httpClient
      .post(new URL(`${this.API_PATH}/`, this.#storageService.get(StorageKeys.API_URL)!).href, ticket)
      .pipe(
        map((res: any) => {
          return <Ticket>res;
        })
  ...
  public cancel(ticket: Ticket): Observable<Ticket> {
    return this.httpClient
      .post(new URL(`${this.API_PATH}/cancel/`, this.#storageService.get(StorageKeys.API_URL)!).href, {
        id: ticket.id,
        email: ticket.email,
      })
      .pipe(

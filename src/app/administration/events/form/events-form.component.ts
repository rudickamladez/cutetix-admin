...
export class EventsFormComponent {
  ...
  public createEvent() {
    this.eventService
      .create({
        ...
        tickets_sales_end:
          this.form.value.ticketsSalesEnd || new Date(new Date().getDate() + 14).toISOString().substring(0, 16),
      ...
      .subscribe({
        next: event => {
          this.toastr.info("Successfully created.", `Event called '${event.name}'`, {
            progressBar: true,
          });
  ...
  public editEvent() {
    this.eventService
      ...
      .subscribe({
        next: event => {
          this.toastr.info("Successfully edited.", `Event called '${event.name}'`, {
            progressBar: true,
          });
          this.router.navigate(["/events/detail/" + event.id]);
        },

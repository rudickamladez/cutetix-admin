...
import type { TicketGroup, TicketGroupSum, TicketGroupUpdate } from "./ticket_groups.types";
...
export class TicketGroupService {
  ...
  public create(ticket_group: TicketGroup): Observable<TicketGroup> {
    return this.httpClient
      .post(new URL(`${this.API_PATH}/`, this.#storageService.get(StorageKeys.API_URL)!).href, ticket_group)
      .pipe(
        map((res: any) => {
          return <TicketGroup>res;
        })
  ...
  public update(id: string, body: TicketGroupUpdate): Observable<TicketGroup> {
    return this.httpClient
      .put(new URL(`${this.API_PATH}/${id}/`, this.#storageService.get(StorageKeys.API_URL)!).href, body)
      .pipe(
        map((res: any) => {
          return <TicketGroup>res;
        })
  ...
  public delete(id: string): Observable<TicketGroup> {
    return this.httpClient
      .delete(new URL(`${this.API_PATH}/${id}/`, this.#storageService.get(StorageKeys.API_URL)!).href)
      .pipe(
        map((res: any) => {
          return <TicketGroup>res;
        })
  ...
  public getActiveSum(): Observable<TicketGroupSum> {
    return this.httpClient
      .get(new URL(`${this.API_PATH}/active/sum/`, this.#storageService.get(StorageKeys.API_URL)!).href)

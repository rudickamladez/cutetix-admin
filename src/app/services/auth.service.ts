...
export class AuthService implements OnDestroy {
  ...
  login(username: string, password: string): void {
    ...
    this.#loginSub = this.#http
      .post<TokensFromApi>(
        new URL("auth/login", this.#storageService.get(StorageKeys.API_URL)!)
          .href,
        body.toString(),
        {
          headers: {
      ...
      .subscribe({
        ...
        error: (err: HttpErrorResponse) => {
          ...
          console.error(err);
          if (err.error.detail) {
            this.#toastr.error(err.error.detail, "Login");
            return;
          }
          this.#toastr.error(err.statusText, "Login");
        },
      });
  }
  ...
  refresh(): void {
    ...
    if (
      ...
      !this.#visibilityService.visible()
    ) {
      return;
    }
  ...
  logout(): void {
    ...
    this.#http
      .post(new URL("auth/logout", apiUrl).href, {
        refresh_token: refreshToken,
      })
      .pipe(timeout(1000))
      .subscribe({
  ...
  #scheduleNextRefresh(): void {
    try {
      ...
      if (parsedAccessToken.exp !== undefined) {
        ...
        this.#logging.log(
          "auth",
          `Next token refresh in ${refreshTime / 1000} s.`
        );
  ...
  #handleRefreshLock(): void {
    ...
    navigator.locks
      ...
      .catch(() => {
        this.#logging.log(
          "auth",
          "This window will not refresh access tokens."
        );
  ...
  isLoggedIn(): boolean {
    ...
    try {
      const parsedAccessToken = jwtDecode(accessToken);
      return (
        parsedAccessToken.exp === undefined ||
        parsedAccessToken.exp * 1000 > Date.now()
      );
    } catch (err) {
      return false;
    }
  ...
  #refreshAccessTokenOnWS(): void {
    this.#logging.log(
      "auth",
      "NOW SHOULD Refresh access token on WS, but it is NOT IMPLEMENTED right now."
    );

...
export class AuthService implements OnDestroy {
  ...
  refresh(): void {
    ...
    try {
      this.#http
        .post<TokensFromApi>(new URL("auth/refresh", this.#storageService.get(StorageKeys.API_URL)!).href, {
  ...
  logout(): void {
    ...
    this.#http
      .post(new URL("auth/logout", apiUrl).href, { refresh_token: refreshToken })
      .pipe(timeout(1000))
      .subscribe({
  ...
  #handleRefreshLock() {
    ...
    navigator.locks
      .request(LockNames.REFRESH_LOCK, { signal: this.#lockAbortController.signal }, () => {
